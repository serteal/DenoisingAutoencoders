import torch
import numpy as np
from tabdae.models.network import (
    EntityEmbedder,
    OneHotEncoder,
    SwapNoiseCorrupter,
    MLP,
...
def test_ae():
    ...
    m = AutoEncoder(datatype_info, cards, -1, 'onehot', MLP, dict(hidden_size=128))
    ...
    m = AutoEncoder(datatype_info, cards, 2, 'embed', MLP, dict(hidden_size=128))
    rec, msk = m({'bins': x_bins, 'cats': x_cats, 'nums': x_nums})
    assert msk.shape == torch.Size([3200, 8])
    assert rec['bins'].shape == torch.Size([3200, 1])
    assert rec['cats'][0].shape == torch.Size([3200, 4])
